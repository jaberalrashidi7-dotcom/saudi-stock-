...
if symbol:
    # إضافة .SR لتتوافق مع مكتبة Yahoo Finance للسوق السعودي
    ...
    with st.spinner('جاري جلب البيانات...'):
        ...
        if not data.empty:
            ...
            # 5. تقييم السهم بناءً على معادلة بيتر لينش
            ...
            if eps and eps > 0 and (growth_rate > 0 or dividend_yield > 0):
                ...
                st.warning("لا تتوفر بيانات كافية (أو أن ربحية السهم بالسالب) لحساب السعر العادل وفق نموذج بيتر لينش.")

            st.divider()
            
            # 6. تقييم السهم بناءً على معادلة بنجامين جراهام
            ...
            # 7. تقييم السهم بناءً على نموذج التدفقات النقدية المخصومة (DCF)
            ...
            growth_rate_dcf = col_dcf2.number_input("معدل نمو التدفقات المتوقع (5 سنوات قادمة) (%)", value=5.0, step=0.5) / 100
            ...
            if fcf and shares and fcf > 0:
                ...
                st.warning("لم نتمكن من جلب بيانات التدفق النقدي الحر بشكل تلقائي لهذا السهم لتطبيق نموذج DCF.")

            st.divider()
            
            # 8. حاسبة تعديل التكلفة الذكية (العكسية)
            ...
            if st.button("احسب الكمية المطلوبة"):
                ...
                if denominator == 0:
                    ...
                    if required_shares > 0:
                        ...
                        st.metric("التكلفة الإضافية المطلوبة للتعديل", f"{total_cost:,.2f} ريال")
                    else:
